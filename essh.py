...
def get_my_ip():
    host_name = ([(s.connect(('8.8.8.8', 53)), s.getsockname()[0], s.close()) for s in
            ...
            for packet in packets:
                ...
                if (packet_flags == 18) or (packet_flags == 24):
                    if tuple_key not in established_sessions:
                        established_sessions[tuple_key] = {'ip1->ip2': [], 'ip2->ip1': []}
...
def is_successful_login(size):
    if size >= 2000:
        return f"Successful SSH connection with session size {size}"
    else:
        return f"Not successful SSH connection with session size {size}"


if __name__ == '__main__':

    parser = argparse.ArgumentParser(description="Program 0.1 version {}. Author: xxx. ".format(__version__),
                                     usage='%(prog)s -n <screen_name> [options]')

    parser.add_argument('-f', '--filename', help='Output file with the amount of packets per port in json format.',
                        action='store',
                        required=False)
    parser.add_argument('-i', '--interface', help='Interface name to sniff the packets from', type=str, action='store',
                        required=False)
    parser.add_argument('-r', '--read', help='Enter pcap path to read', type=str, action='store',
                        required=False)
    parser.add_argument('-s', '--sniff', help='Put the IP and PORT to sniff on', type=str, action='store',
                        required=False)
    ...
    for session_size in session_size.values():
        total_session_size = sum(session_size.values())
        print(f'Session {session_size} total session size {total_session_size} ')
